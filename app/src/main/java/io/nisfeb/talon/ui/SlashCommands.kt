...
suspend fun runCommand(
    ...
    return when (parsed.cmd) {
        ...
        "hn" -> runHn(http)
        "loc" -> runLoc(parsed.args, context)
...
private suspend fun runLoc(
    ...
    if (!hasLocationPermission(context)) {
        return CommandResult.Error(
            "/loc: grant Location permission (System Settings → Apps → Talon → Permissions)"
        )
    }
    val loc = fetchLastKnownLocation(context)
        ?: return CommandResult.Error("/loc: no location fix available yet — try again in a moment")
    return CommandResult.Send(formatLocationShare(loc.latitude, loc.longitude))
}
