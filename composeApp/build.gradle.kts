...
kotlin {
    ...
    sourceSets {
        commonMain {
            // Pull the generated TalonBuild constants into commonMain
            // so SettingsScreen.About can render the version on every
            // target without an expect/actual hop. The explicit task
            // wiring below makes every Kotlin compile task wait on
            // generation; the srcDir entry just tells KGP where to
            // find the sources.
            kotlin.srcDir(
                layout.buildDirectory.dir("generated/talon-build/commonMain/kotlin"),
            )
        }
        commonMain.dependencies {
            ...
            // The core icon set, at the version the Compose plugin pins its
            // icon artifacts to (no accessor exists for core). The extended
            // set is a desktop TEST dependency only: the 46 extended icons
            // the app uses are owned in ui/icons/TalonIcons.kt, which keeps
            // 18 MB of Kotlin out of the iOS release link.
...
// SettingsScreen would have no portable way to read the version —
// Android can ask PackageManager but desktop has no equivalent.
// Single source of truth: the two `talonVersion*` literals above.
val generateTalonBuild = tasks.register("generateTalonBuild") {
    ...
    doLast {
        ...
        pkgDir.mkdirs()
        pkgDir.resolve("TalonBuild.kt").writeText(
            """
...
// Make every Kotlin compile task wait on the version-file generator.
// Without this, a clean build can fail because compileKotlinDesktop
// runs before TalonBuild.kt has been written. Using `withType` covers
// every target's Kotlin compile (Android variants + desktop) without
