...
/**
 * The icons Talon takes from the extended Material set, by the name
 * TalonIcons gives each. This is the one place the extended library
 * is still named: it is a desktop TEST dependency now, so the
 * generator can read these and the fidelity test can compare against
 * them, while the app itself links only the core set.
 */
object TalonIconSources {
