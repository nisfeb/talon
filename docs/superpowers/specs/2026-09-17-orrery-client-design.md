...
Once a day, from 07:00 until noon in the owner's zone (`person/me.timezone`, else America/New_York), the install feeding orrery mails the owner, from the owner, through auspex: subject `Daily brief <date>`, plain text in short lines. It needs a frontier model under AI; without one, or when the ship, the calendar or the model does not answer, nothing is sent and the log says why. A brief already in the owner's mail for that day, sent by another install, is not sent again.
...
The owner's reply is read once, keyed on its message id (`reply:<id>`), and recorded only after every write is done. Quoted lines, the attribution and anything after it, and any line the brief itself said are stripped first, so the brief's text is never read back as facts. A sentence naming a tag is a direction: approve, dismiss or done move the action (a proposal is approved on its way to done), and a new due or subject ("A3 due friday", "A3 make it about Linus") dismisses the action and proposes a replacement with only the fields `/act` takes, since the action API moves a status and nothing else. Everything else is triaged with `common/analyst-prompt.md` word for word, by the model message triage uses (this device's unless the cloud is opted in), so a health fact in a reply never goes to a hosted model by default. Bodies the answer would make are resolved against the ship first; observations are held to the schema's attributes and to bodies that exist, carry conf 100 and source `{"kind": "mail", "id": <reply id>}`. A reply with facts and no model to read them waits, whole. Every model call's tokens and cost go to the log.
